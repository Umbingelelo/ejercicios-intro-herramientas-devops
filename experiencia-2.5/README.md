...
1. <https://awsacademy.instructure.com> → iniciar sesión.
2. Curso → **Modules → Learner Lab**.
3. Botón **Start Lab** y esperar a que el círculo quede verde.
4. Click en **AWS** para abrir la consola.
5. Esquina superior derecha: la región debe decir **us-east-1
...
1. En la consola AWS (con el lab abierto): **ECR → Repositories → Create**.
2. Crear **tres** repositorios privados en la región `us-east-1`:
...
3. Para cada uno, copien el **URI** (formato
   `123456789012.dkr.ecr.us-east-1.amazonaws.com/casino-frontend`).
4. Anoten su **AWS Account ID** (los 12 dígitos del URI).
...
1. **Capturen evidencia** (screenshots) de:
...
| **IE7.** Manejo de Secrets (todos los `AWS_*`, `EC2_*`, `*_HOST_PRIVATE`, `DB_*`, `JWT_SECRET`) | Todo en Secrets | Algún valor en texto plano | Credenciales en repo | 10% |
